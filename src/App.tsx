      ...
      >
        ...
        <Slide className="overflow-y-auto no-scrollbar">
          <div className="w-full max-w-6xl mx-auto flex flex-col lg:flex-row items-center justify-center lg:justify-between gap-6 sm:gap-12 z-10 py-10 sm:py-16 px-4 pr-12 sm:pr-20">
            <div className="w-full lg:flex-1 space-y-4 sm:space-y-6 font-mono bg-[#111]/90 backdrop-blur-md p-5 sm:p-10 border-2 border-[#00ff00] brutalist-shadow-green">
              ...
              <ScrollReveal delay={0.4}><div className="bg-[#ff00ff] text-black font-display font-black text-sm sm:text-2xl lg:text-3xl p-3 sm:p-4 my-4 sm:my-6 brutalist-shadow-purple transform -rotate-1 hover:rotate-0 transition-transform cursor-default">«ты вообще понимаешь, что сделал?»</div></ScrollReveal>
        ...
        <Slide className="overflow-y-auto no-scrollbar">
          <div className="w-full max-w-6xl mx-auto z-10 font-mono flex flex-col items-center justify-center text-center py-10 sm:py-16 px-4 pr-10 sm:pr-20 relative min-h-[100dvh]">
            ...
            <ScrollReveal delay={1.2}><div className="bg-[#ff00ff] text-black font-display font-black text-sm sm:text-2xl lg:text-4xl p-3 sm:p-6 mt-4 brutalist-shadow-green transform -rotate-1 inline-block z-30 relative hover:rotate-1 transition-transform">ПОЗДРАВЛЯЮ ТЕБЯ С ЭТИМ ПРЕКРАСНЫМ ДНЁМ!!!!</div></ScrollReveal>
          </div>
        </Slide>
...
function ScrollReveal({ children, delay = 0, rootMargin = "0px" }: { children: React.ReactNode, delay?: number, rootMargin?: string }) {
  return (
    <motion.div initial={{ opacity: 0, x: -20, filter: "blur(10px)" }} whileInView={{ opacity: 1, x: 0, filter: "blur(0px)" }} viewport={{ once: true, margin: rootMargin as any }} transition={{ duration: 0.8, delay, ease: "easeOut" }}>
      {children}
    </motion.div>
  );
}
...
function StylizedCake() {
  ...
  return (
    <group ref={groupRef} position={[0, -1.5, 0]}>
      ...
      <Text position={[0, 3.8, 0]} fontSize={1.5} color="#ffffff" outlineWidth={0.05} outlineColor="#ff00ff" font="https://fonts.gstatic.com/s/inter/v12/UcCO3FwrK3iLTeHuS_fvQtMwCp50KnMw2boKoduKmMEVuGKYMZhrib2Bg-4.ttf" fontWeight="900">30</Text>
